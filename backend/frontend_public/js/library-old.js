                        ...
                        if (universalEditModal) {
                            ...
                            universalEditForm.onsubmit = async (ev) => {
                                ...
                                try {
                                    ...
                                    const res = await fetch(`https://judahcareacademy.onrender.com/api/library/${bookId}`, {
                                        method: 'PUT',
                                        headers: {
                ...
                // Issue Book
                    ...
                    universalForm.onsubmit = async (e) => {
                        ...
                        try {
                            ...
                            const response = await fetch(`https://judahcareacademy.onrender.com/api/library/${bookId}/issue`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': `Bearer ${token}`
...
if (libraryForm) {
    libraryForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const res = await fetch('https://judahcareacademy.onrender.com/api/library', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
            ...
            universalConfirmYes.onclick = async () => {
                ...
                for (const bookId of selectedBookIds) {
                    try {
                        ...
                        await fetch(`https://judahcareacademy.onrender.com/api/library/${bookId}`, {
                            method: 'DELETE',
                            headers: { 'Authorization': `Bearer ${token}` }
                        });
                    } catch {}
...
if (libraryBulkExport) {
    libraryBulkExport.onclick = async function() {
        if (selectedBookIds.size === 0) return;
        const token = localStorage.getItem('token')
        let url = 'https://judahcareacademy.onrender.com/api/library';
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${token}` } });
        const books = await res.json();
