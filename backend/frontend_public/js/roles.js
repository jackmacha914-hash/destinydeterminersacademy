...
async function loadRolesWithFilters() {
    ...
    let url = 'https://judahcareacademy.onrender.com/api/roles' + buildRolesQueryString(filters);
    try {
...
if (roleForm) {
    roleForm.addEventListener('submit', async (e) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const res = await fetch('https://judahcareacademy.onrender.com/api/roles', {
                method: 'POST',
                headers: {
    ...
    roleTableBody.addEventListener('click', async (e) => {
        ...
        if (btn.classList.contains('edit-role-btn')) {
            ...
            universalEditForm.onsubmit = async (ev) => {
                ...
                try {
                    const res = await fetch(`https://judahcareacademy.onrender.com/api/roles/${roleId}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${token}`,
...
if (rolesBulkExport) {
    rolesBulkExport.onclick = async function() {
        if (selectedRoleIds.size === 0) return;
        const token = localStorage.getItem('token');
        let url = 'https://judahcareacademy.onrender.com/api/roles';
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${token}` } });
        const roles = await res.json();
