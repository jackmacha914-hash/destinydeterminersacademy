...
async function loadClasses() {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://destinydeterminersacademy.onrender.com/api/classes/teacher', {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function searchStudents(query) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://destinydeterminersacademy.onrender.com/api/users/students?search=${encodeURIComponent(query)}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function addSelectedStudents() {
    try {
        ...
        const response = await fetch(`https://destinydeterminersacademy.onrender.com/api/classes/${classId}/students`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
