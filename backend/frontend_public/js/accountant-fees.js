...
class AccountantFees {
    ...
    initialize() {
        ...
        this.tryDeleteFee = async (feeId, token) => {
            ...
            try {
                const deleteUrl = `https://destinydeterminersacademy.onrender.com/api/fees/${feeId}`;
...
if (this.feesYearFilter) {
    ...
    async loadFeesWithFilters() {
        ...
        try {
           ...
           if (yearFilter) params.append('academicYear', yearFilter);
            ...
            const apiUrl = `https://destinydeterminersacademy.onrender.com/api/fees?${params.toString()}`;
    ...
    try {
        ...
        const updatedFee = (await response.json()).fee || await response.json();
...
// Keep your showPaymentModal helper as is
...
async showPaymentModal(fee, balance) {
    ...
    async getFeeById(feeId) {
        try {
            ...
            const response = await fetch(`https://destinydeterminersacademy.onrender.com/api/fees/${feeId}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
