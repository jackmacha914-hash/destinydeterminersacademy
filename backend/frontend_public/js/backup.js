...
async function loadBackups() {
    ...
    try {
        const res = await fetch('https://destinydeterminersacademy.onrender.com/api/backups', {
...
backupList.addEventListener('click', async (e) => {
    ...
    if (btn.classList.contains('download-backup-btn')) {
        window.open(`https://destinydeterminersacademy.onrender.com/api/backups/${backupId}/download`, '_blank');
    }
    // Restore Backup
    else if (btn.classList.contains('restore-backup-btn')) {
        if (confirm('Are you sure you want to restore this backup? This will overwrite current data.')) {
            try {
                const res = await fetch(`https://destinydeterminersacademy.onrender.com/api/backups/${backupId}/restore`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (res.ok) {
