...
async function removeClubMember(clubId, memberId, memberName) {
    ...
    try {
        ...
        const url = `https://judahcareacademy.onrender.com/api/clubs/${encodedClubId}/members/${encodedMemberId}`;
...
async function loadClubMembers(clubId, clubName) {
    ...
    try {
        ...
        const response = await fetch(`https://judahcareacademy.onrender.com/api/clubs/${clubId}/students`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
...
async function addStudentsToClub(studentIds) {
    ...
    try {
        const endpoint = `https://judahcareacademy.onrender.com/api/clubs/${currentClubId}/students`;
...
async function loadClubsWithFilters() {
    ...
    let url = 'https://judahcareacademy.onrender.com/api/clubs' + buildClubQueryString(filters);
    try {
    ...
    clubsBulkDelete.onclick = async function() {
        ...
        universalConfirmYes.onclick = async () => {
            ...
            for (const clubId of selectedClubIds) {
                try {
                    await fetch(`https://judahcareacademy.onrender.com/api/clubs/${clubId}`, {
                        method: 'DELETE',
...
if (clubsBulkExport) {
    clubsBulkExport.onclick = async function() {
        if (selectedClubIds.size === 0) return;
        const token = localStorage.getItem('token');
        let url = 'https://judahcareacademy.onrender.com/api/clubs';
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${token}` } });
        const clubs = await res.json();
...
async function handleClubListClick(e) {
    ...
    if (btn.classList.contains('edit-club-btn')) {
        ...
        universalEditForm.onsubmit = async (ev) => {
            ...
            try {
                const res = await fetch(`https://judahcareacademy.onrender.com/api/clubs/${clubId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
